...
class CPPGPID(PPO):
    ...
    def learn(self) -> tuple[float, float, float]:
        ...
        for phase in range(self._number_phases):
            ...
            if self._cfgs.algo_cfgs.joint_aux_phase:
                print('starting joint aux phase')
                for aux_phase in range(self._cfgs.algo_cfgs.E_aux):
                    self._aux_update()
            else:
                print('starting disjoint aux phase')
                for aux_phase in range(self._cfgs.algo_cfgs.E_aux):
                    self._aux_reward_update()
                for aux_phase in range(self._cfgs.algo_cfgs.E_aux):
                    self._aux_cost_update()
    ...
    def _aux_reward_update(self) -> None:
        data = self._buf.get_aux_data()
        dataset_size = data["obs"].shape[0]
        batch_size = dataset_size // (self._cfgs.algo_cfgs.N_pi * self._cfgs.algo_cfgs.number_aux_mb_per_Npi)
        ...
        for (obs, target_value_r, target_value_c, mean_old, std_old) in dataloader:
            ...
            L_joint = L_reward + torch.exp(self._log_beta_clone.detach()) * kl
            self._actor_critic.actor_optimizer.zero_grad()
            L_joint.backward()
            self._actor_critic.actor_optimizer.step()
    ...
    def _aux_cost_update(self) -> None:
        data = self._buf.get_aux_data()
        dataset_size = data["obs"].shape[0]
        batch_size = dataset_size // (self._cfgs.algo_cfgs.N_pi * self._cfgs.algo_cfgs.number_aux_mb_per_Npi)
        ...
        for (obs, target_value_r, target_value_c, mean_old, std_old) in dataloader:
            ...
            L_joint = self._cfgs.algo_cfgs.alpha_cost * L_cost + torch.exp(self._log_beta_clone.detach()) * kl
            self._actor_critic.actor_optimizer.zero_grad()
            L_joint.backward()
            self._actor_critic.actor_optimizer.step()
